...
현재 loop 값은 `project-init`, `project-plan`, `project-run`, `project-review`만 허용한다. 종료/예외 결과는 별도 outcome으로 `COMPLETE`, `REPLAN`, `BLOCKED`, `DEFERRED_BACKLOG`를 사용한다.
...
| project-review | plan, acceptance/validation matrix, run evidence | criterion별 판정, remediation packet 또는 closeout report, backlog | 독립 증거 기반 평가 및 회귀 판단 | 모든 기준 통과면 `COMPLETE`; 미충족이면 `REPLAN -> project-plan`; 위험/한도 초과면 `BLOCKED` | rubric 결과, criterion IDs, evidence refs |
...
Review outcome은 다음 네 가지다.

- `COMPLETE`: 승인된 모든 수용 기준과 필수 validation evidence가 통과하고 critical/고심각도 결함이 없다.
- `REPLAN`: 현재 수용 기준을 침해하는 미해결 criterion이 있다. 이 경우 오직 `project-plan`으로 회귀한다.
...
Ouroboros interview는 명시적으로 요청된 작업에서만 사용한다. Superpowers planning/execution도 task-scoped explicit opt-in이 있을 때만 사용한다. opt-in이 없거나 도구가 없으면 동등한 산출물 계약을 지키는 일반 Codex 경로를 사용한다.
