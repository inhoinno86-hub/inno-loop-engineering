...
def main(argv=None):
    ...
    for name in ("plan", "run", "replan", "retry-run"): sub.add_parser(name).add_argument("--evidence", required=True)
    ...
    execution_remediation=sub.add_parser("record-execution-remediation-packet"); execution_remediation.add_argument("--artifact", required=True)
    ...
    try:
        if args.command in ("init", "init-auto"):
            ...
            elif args.command=="record-execution-remediation-packet": state["execution_remediation_packet"] = core.record_execution_remediation_packet(root,state,args.artifact)
            ...
            elif args.command=="retry-run": core.transition(state,"retry-run",args.evidence)
