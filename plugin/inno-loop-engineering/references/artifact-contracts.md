...
Every state record has `run_id`, `input_hash`, `artifact_version`, `checkpoint`, `lifecycle_authorization`, `last_review_outcome`, `replan_history`, `max_replans`, `decision_log`, `assumption_log`, `verification_evidence`, `integration_evidence`, `input_packets`, `quality_gates`, `init_outputs`, `plan_revisions`, `review_artifact`, and `remediation_packet`. Each run stores state and artifacts under `.loop-engine/runs/<run-id>/`; `current.json` selects the active run.
