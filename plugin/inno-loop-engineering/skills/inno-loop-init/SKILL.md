...
Use only validated input. The command creates `.loop-engine/state.json` in `project-init`. If input validation fails, report `BLOCKED`; do not attempt to bypass it.
